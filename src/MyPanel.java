...
public class MyPanel extends JPanel {
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				if (!isDragging) {
					...
					if(!(checkColisionNode((int)pointStart.getX(), (int)pointStart.getY()) ||
					checkColisionNode((int)pointEnd.getX(), (int)pointEnd.getY()))) {
	...
	private boolean checkColisionNode(int targetX, int targetY){
		int x,y;
		for(Node it : listaNoduri){
			x = it.getCoordX();
			y = it.getCoordY();
			if((float)Math.sqrt(Math.pow(targetX - x, 2) + Math.pow(targetY - y, 2)) <= node_diam / 2)
				return false;
		}
		return true;
	}
